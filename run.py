...
from ibkr_shared import get_open_markets, load_dotenv
    ...
    try:
        while True:
            ...
            if not open_markets:
                ...
                print(
                    f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] Prochaine verification: {next_run.strftime('%Y-%m-%d %H:%M:%S')} UTC"
                )
            ...
            next_run = datetime.now(timezone.utc) + timedelta(hours=1)
            print(
                f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] Prochaine execution: {next_run.strftime('%Y-%m-%d %H:%M:%S')} UTC"
            )
            print("En attente pendant 1 heure...")
